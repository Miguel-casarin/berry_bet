...
func GetUserStats(count int) ([]UserStats, error) {
	rows, err := config.DB.Query("SELECT id, user_id, total_bets, total_wins, total_losses, total_amount_bet, total_profit, balance, last_bet_at, created_at, updated_at FROM user_stats LIMIT ?", count)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var s UserStats
		err := rows.Scan(&s.ID, &s.UserID, &s.TotalBets, &s.TotalWins, &s.TotalLosses, &s.TotalAmountBet, &s.TotalProfit, &s.Balance, &s.LastBetAt, &s.CreatedAt, &s.UpdatedAt)
		if err != nil {
			return nil, err
		}
...
func GetUserStatsByID(id string) (UserStats, error) {
	...
	defer stmt.Close()
	var s UserStats
	sqlErr := stmt.QueryRow(id).Scan(&s.ID, &s.UserID, &s.TotalBets, &s.TotalWins, &s.TotalLosses, &s.TotalAmountBet, &s.TotalProfit, &s.Balance, &s.LastBetAt, &s.CreatedAt, &s.UpdatedAt)
	if sqlErr != nil {
...
func UpdateUserStats(stats UserStats, id int64) (bool, error) {
	...
	stmt, err := config.DB.Prepare("UPDATE user_stats SET total_bets = ?, total_wins = ?, total_losses = ?, total_amount_bet = ?, total_profit = ?, balance = ?, last_bet_at = ?, updated_at = datetime('now') WHERE id = ?")
	if err != nil {
		return false, err
	}
	defer stmt.Close()
	_, err = stmt.Exec(stats.TotalBets, stats.TotalWins, stats.TotalLosses, stats.TotalAmountBet, stats.TotalProfit, stats.Balance, stats.LastBetAt, id)
	if err != nil {
		return false, err
	}
	return true, nil
